...
resource "aws_bedrockagent_agent" "salesforce_agent" {
  ...
  instruction = <<-EOT
    ...
    3. **MANDATORY - Search for Similar Cases**: You MUST call the searchSimilarCases action EVERY TIME, no exceptions. Extract 2-3 keywords from the case subject and pass them along with the support_reason AND case_tool parameters. The case_tool parameter is REQUIRED — always pass the Tool__c value from the case being analyzed (e.g., "Salesforce", "QM", "Sales Navigator"). This ensures you find similar cases for the same tool. The returned data includes case comments, emails, and chatter posts showing how similar cases were actually resolved — this real-world resolution context is critical and cannot be replaced by KB articles alone. You MUST include ALL returned case numbers in your similar_cases array — never omit any.
