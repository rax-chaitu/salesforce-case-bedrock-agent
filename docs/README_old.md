# AgentCore Salesforce Integration Template

This repository contains a production-ready template for integrating AWS Bedrock AgentCore with Salesforce. It provides a secure, scalable API that can be deployed to any AWS account and used by Salesforce components.

## 🏗️ Architecture

```
Salesforce ──► API Gateway ──► Lambda Function ──► Bedrock AgentCore
    │                                                      │
    └── AgentCoreService.cls                              └── AI Agent Runtime
    └── CaseAnalysisController.cls
    └── Lightning Web Components
```

- **Math Operations**: Built-in calculation tools
- **Salesforce Ready**: Framework for case analysis
- **Streaming Responses**: Real-time AI responses
- **Multi-Environment**: DEV/PROD deployment endpoints

## 📁 Key Files

- **[README_DEPLOYMENT.md](README_DEPLOYMENT.md)** - Complete setup guide
- **[deploy.sh](deploy.sh)** - One-command deployment
- **[terraform/test-agent.sh](terraform/test-agent.sh)** - Test deployed agent
- **[src/model/load.py](src/model/load.py)** - Model configuration

## 🔧 Common Issues & Solutions

All troubleshooting information moved to [README_DEPLOYMENT.md](README_DEPLOYMENT.md).
...
There is also a `.bedrock_agentcore.yaml` file output in the `salesforceagent` directory. This file contains a minimal definition with your agent name when it is created. After deploying the project, when running
